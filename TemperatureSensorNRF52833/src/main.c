...
#define TEMP_SENSOR_INIT_BASIC_LOCATION_DESC   "Room temperature"
...
#define TEMP_SENSOR_INIT_BASIC_PH_ENV          ZB_ZCL_BASIC_ENV_UNSPECIFIED
...
} temperature_sensor_device_ctx_t;

/* Zigbee device application context storage. */
static temperature_sensor_device_ctx_t dev_ctx;
...
//}
static void temperature_measure_set_value(zb_int16_t min_value, zb_int16_t max_value, zb_uint16_t tolerance)
{
	LOG_INF("Set min temperature value: %i", min_value);
	LOG_INF("Set max temperature value: %i", max_value);
	LOG_INF("Set tolerance value: %i", tolerance);
 ...
 */
static void temperature_sensor_clusters_attr_init(void)
{
	/* Basic cluster attributes data */
	dev_ctx.basic_attr.zcl_version   = ZB_ZCL_VERSION;
	dev_ctx.basic_attr.app_version   = TEMP_SENSOR_INIT_BASIC_APP_VERSION;
	dev_ctx.basic_attr.stack_version = TEMP_SENSOR_INIT_BASIC_STACK_VERSION;
	dev_ctx.basic_attr.hw_version    = TEMP_SENSOR_INIT_BASIC_HW_VERSION;
	...
	ZB_ZCL_SET_STRING_VAL(
		dev_ctx.basic_attr.mf_name,
		TEMP_SENSOR_INIT_BASIC_MANUF_NAME,
		ZB_ZCL_STRING_CONST_SIZE(TEMP_SENSOR_INIT_BASIC_MANUF_NAME));

	ZB_ZCL_SET_STRING_VAL(
		dev_ctx.basic_attr.model_id,
		TEMP_SENSOR_INIT_BASIC_MODEL_ID,
		ZB_ZCL_STRING_CONST_SIZE(TEMP_SENSOR_INIT_BASIC_MODEL_ID));

	ZB_ZCL_SET_STRING_VAL(
		dev_ctx.basic_attr.date_code,
		TEMP_SENSOR_INIT_BASIC_DATE_CODE,
		ZB_ZCL_STRING_CONST_SIZE(TEMP_SENSOR_INIT_BASIC_DATE_CODE));

	dev_ctx.basic_attr.power_source = TEMP_SENSOR_INIT_BASIC_POWER_SOURCE;

	ZB_ZCL_SET_STRING_VAL(
		dev_ctx.basic_attr.location_id,
		TEMP_SENSOR_INIT_BASIC_LOCATION_DESC,
		ZB_ZCL_STRING_CONST_SIZE(TEMP_SENSOR_INIT_BASIC_LOCATION_DESC));

	dev_ctx.basic_attr.ph_env = TEMP_SENSOR_INIT_BASIC_PH_ENV;
	...
	//	ZB_FALSE);
        dev_ctx.temperature_measure_attr.min_measure_value = ZB_ZCL_TEMP_MEASUREMENT_MIN_VALUE_DEFAULT_VALUE;
        dev_ctx.temperature_measure_attr.max_measure_value = ZB_ZCL_TEMP_MEASUREMENT_MAX_VALUE_DEFAULT_VALUE;
        dev_ctx.temperature_measure_attr.tolerance = ZB_ZCL_ATTR_TEMP_MEASUREMENT_TOLERANCE_MIN_VALUE;
        dev_ctx.temperature_measure_attr.measure_value = ZB_ZCL_TEMP_MEASUREMENT_VALUE_DEFAULT_VALUE;
}
...
void main(void)
{
	...
	LOG_INF("Starting ZBOSS Temperature Sensor");
	...
	temperature_sensor_clusters_attr_init();
	//level_control_set_value(dev_ctx.level_control_attr.current_level);
        temperature_measure_set_value(dev_ctx.temperature_measure_attr.min_measure_value, dev_ctx.temperature_measure_attr.max_measure_value, dev_ctx.temperature_measure_attr.tolerance);

	/* Start Zigbee default thread */
	zigbee_enable();

	LOG_INF("ZBOSS Temperature Sensor started");
