...
static void _drawLineByAngle(int16_t x, int16_t y, int16_t angle, uint16_t length, color_t color)
{
	_drawLine(
		x,
		y,
		x + length * (int16_t)cos((angle + _angleOffset) * DEG_TO_RAD),
		y + length * (int16_t)sin((angle + _angleOffset) * DEG_TO_RAD), color);
}
...
{
	_drawLine(
		x + start * (int16_t)cos((angle + _angleOffset) * DEG_TO_RAD),
		y + start * (int16_t)sin((angle + _angleOffset) * DEG_TO_RAD),
		x + (start + length) * (int16_t)cos((angle + _angleOffset) * DEG_TO_RAD),
		y + (start + length) * (int16_t)sin((angle + _angleOffset) * DEG_TO_RAD), color);
}
...
void EPD_drawArc(uint16_t cx, uint16_t cy, uint16_t r, uint16_t th, float start, float end, color_t color, color_t fillcolor)
{
	...
	float astart = fmod(start, _arcAngleMax);
	float aend = fmod(end, _arcAngleMax);
	...
	if (f) {
		_drawLine(cx + (r-th) * (int16_t)cos(astart * DEG_TO_RAD), cy + (r-th) * (int16_t)sin(astart * DEG_TO_RAD),
			cx + (r-1) * (int16_t)cos(astart * DEG_TO_RAD), cy + (r-1) * (int16_t)sin(astart * DEG_TO_RAD), color);
		_drawLine(cx + (r-th) * (int16_t)cos(aend * DEG_TO_RAD), cy + (r-th) * (int16_t)sin(aend * DEG_TO_RAD),
			cx + (r-1) * (int16_t)cos(aend * DEG_TO_RAD), cy + (r-1) * (int16_t)sin(aend * DEG_TO_RAD), color);
	}
}
...
void EPD_drawPolygon(int cx, int cy, int sides, int diameter, color_t color, color_t fill, int rot, uint8_t th)
{
	...
	int* Xpoints=malloc(sides*sizeof(int));
        int* Ypoints=malloc(sides*sizeof(int));							// Set the arrays based on the number of sides entered
	int rads = 360 / sides;										// This equally spaces the points.

	for (int idx = 0; idx < sides; idx++) {
		Xpoints[idx] = cx + (int16_t)sin((float)(idx*rads + deg) * deg_to_rad) * diameter;
		Ypoints[idx] = cy + (int16_t)cos((float)(idx*rads + deg) * deg_to_rad) * diameter;
	}
	...
	if (th) {
		for (int n=0; n<th; n++) {
			if (n > 0) {
				for (int idx = 0; idx < sides; idx++) {
					Xpoints[idx] = cx + (int16_t)sin((float)(idx*rads + deg) * deg_to_rad) * (diameter-n);
					Ypoints[idx] = cy + (int16_t)cos((float)(idx*rads + deg) * deg_to_rad) * (diameter-n);
				}
			}
			for(int idx = 0; idx < sides; idx++) {
				if( (idx+1) < sides)
