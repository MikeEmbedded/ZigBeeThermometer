...
static uint8_t Border[2] = {0x3c, 0x05};			// Border control ( 0x61: white border; 0x51: black border
static uint8_t TempSensor[2] = {0x18, 0x80};			// Border control ( 0x61: white border; 0x51: black border
...
static void vTaskDelay(uint16_t d)
{
  while(d--)
  {
    /* 32 NOPs == 1 usecs */
...
//-----------------------------------------------------------
static void _dma_send(uint8_t *data, uint32_t size)
{
...
//--------------------------------------------------------------------------
static void _direct_send(uint8_t *data, uint32_t len, uint8_t rep)
{
...
//          if (rep == 0)
//            cidx++;	// if not repeating data, increment buffer index
...
// ================================================================
//---------------------------------------------------------------------------
static void SPI_send_data(uint8_t *data, uint32_t len, uint8_t rep)
{
	if (len == 0) return;
...
//	EPD_Write(RamDataEntryMode, sizeof(RamDataEntryMode));	// X increase, Y decrease
	EPD_Write(GDOControl, sizeof(GDOControl));	// Panel configuration, Gate selection
	EPD_Write(Border, sizeof(Border));
	EPD_Write(TempSensor, sizeof(TempSensor));
	EPD_Write(RamDataEntryMode, sizeof(RamDataEntryMode));	// X increase, Y decrease
