...
uint8 zclSampleTemperatureSensor_BattValue = 0;
...
void zclSampleTemperatureSensor_Init( byte task_id )
{
  ...
  bdb_RepAddAttrCfgRecordDefaultToList(SAMPLETEMPERATURESENSOR_ENDPOINT, ZCL_CLUSTER_ID_MS_TEMPERATURE_MEASUREMENT, ATTRID_MS_TEMPERATURE_MEASURED_VALUE, 0, 250, reportableChange);
#endif
  ...
  macRadioSetTxPower(5);
  
  osal_set_event(zclSampleTemperatureSensor_TaskID, HAL_TEMPERATURE_EVENT);
}
...
uint16 zclSampleTemperatureSensor_event_loop( uint8 task_id, uint16 events )
{
  ...
  if( events & HAL_TEMPERATURE_EVENT )
  {
    zclSampleTemperatureSensor_BattValue = HalAdcCheckVddRaw();

    HalLedSet (HAL_LED_1, HAL_LED_MODE_ON);
    
    zclSampleTemperatureSensor_MeasuredValue = readTemperature();
    if (zclSampleTemperatureSensor_MeasuredValue < 0)
    ...
    if (zclSampleTemperatureSensor_MeasuredValue < zclSampleTemperatureSensor_MinMeasuredValue)
      zclSampleTemperatureSensor_MinMeasuredValue = zclSampleTemperatureSensor_MeasuredValue;
    
    if (zclSampleTemperatureSensor_MeasuredValue > zclSampleTemperatureSensor_MaxMeasuredValue)
      zclSampleTemperatureSensor_MaxMeasuredValue = zclSampleTemperatureSensor_MeasuredValue;

    UpdateEink(zclSampleTemperatureSensor_MeasuredValue, zclSampleTemperatureSensor_BattValue, zclSampleTemperatureSensor_NwkState);
    
    osal_start_timerEx(zclSampleTemperatureSensor_TaskID, HAL_TEMPERATURE_EVENT, 30000);
    
    HalLedSet (HAL_LED_1, HAL_LED_MODE_OFF);
    
    // return unprocessed events
    return (events ^ HAL_TEMPERATURE_EVENT);
  }
